...
//		String userNo = session.getUri().getQuery().split("=")[1];
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if (userSessions.containsKey(dto.getSender_no())) {
			...
			ChatMsg entity = ChatMsg.builder().sendMember(member).chatRoom(chatRoom).msgContent(dto.getMsg_content())
					.build();
			// 2.entity를 save
...
//		String userNo = session.getUri().getQuery().split("=")[1];
	...
	// 반환값 : value값
	/**
	 * author: 권용규 history: 2025-03-24 param: url, key data return: value data role:
	 * WebsocketSession url parsing
	 */
	// getQueryParam(session,"senderNo"); -> 3
	// getQueryParam(session,"roomNo"); -> 1
	private String getQueryParam(WebSocketSession session, String key) {
		// 현재 상태 : senderNo=3&roomNo=1
